...
- **⚠️ Rate-limit `/v1/auth/start` — it is the same hole that burned `appfoundry.cc`.** The endpoint is public, unauthenticated, and sends an email on **every** call with no cooldown, no per-IP cap and no CAPTCHA (confirmed 2026-08-12: no rate limiting anywhere in `Backend/src`). Sudoku Crew has open signup just like Markd did, so a trivial script with random addresses would generate hard bounces at scale and burn `sudokucrew.com` exactly the way the old domain went. Two further consequences: Resend free is **100 emails/day**, so ~100 requests locks out every real signup for the day (surfacing as the generic error); and it can be pointed at a third party's inbox as an email-bombing vector.

  **Do first — no code, no deploy** (which matters while npm is broken): a **Cloudflare Rate Limiting rule** on path `/v1/auth/start`, ~5 requests per IP per minute, from the dashboard.

  **Then, in the Worker:** a per-email cooldown — the `auth_codes` row already carries `expires_at`, so refuse a re-send if one was issued in the last ~60s and return a distinct error the client can show. Pairs naturally with the "Resend code" button below, which needs a cooldown regardless. **Also:** a Resend bounce webhook to stop repeat sends to addresses already known bad, so one typo can't compound.
